...
gif_path = "IMG_8286.gif"
gif_exists = os.path.exists(gif_path)

# Custom CSS for styling with GIF transition and black bold text
st.markdown("""
    <style>
    ...
    .gif-transition {
        ...
        background: linear-gradient(135deg, #000000, #333333); /* Fallback gradient */
        display: flex;
        align-items: center;
        justify-content: center;
    }
    .gif-transition.active {
        opacity: 1;
        pointer-events: auto;
    }
    .gif-transition img {
    ...
    <!-- GIF Transition Container -->
    <div class="gif-transition" id="gifTransition">
        <img id="transitionGif" src="IMG_8286.gif" alt="Transition" style="display: none;">
    </div>
    
    <!-- Fallback Transition -->
    <div class="fallback-transition" id="fallbackTransition"></div>
    
    <!-- Loading Spinner -->
    <div class="loading-spinner" id="loadingSpinner"></div>
    
    <script>
        // GIF transition control
        function showTransition(callback) {
            const gifTransition = document.getElementById('gifTransition');
            const fallbackTransition = document.getElementById('fallbackTransition');
            ...
            if (transitionGif.complete && transitionGif.naturalHeight !== 0) {
                // GIF is loaded and valid
                transitionGif.style.display = 'block';
                gifTransition.classList.add('active');
                ...
                // Get GIF duration (approximate - GIFs don't have built-in duration detection)
                // Adjust this value based on your GIF's actual duration
                const gifDuration = 2500; // 2.5 seconds - adjust as needed
                
                setTimeout(() => {
                    gifTransition.classList.remove('active');
                ...
                // GIF failed to load, use fallback
                console.warn('GIF not loaded, using fallback transition');
                fallbackTransition.classList.add('active');
                ...
                setTimeout(() => {
                    fallbackTransition.classList.remove('active');
                    loadingSpinner.classList.remove('active');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Preload GIF
            transitionGif.onload = function() {
                console.log('GIF loaded successfully');
            };
            
            transitionGif.onerror = function() {
        ...
        const observer = new MutationObserver((mutations) => {
            const now = Date.now();
            if (now - lastRerunTime > 100) { // Prevent multiple triggers
                lastRerunTime = now;
...
# Test GIF loading (hidden)
if gif_exists:
    try:
        # This will help preload the GIF and catch any loading issues
        with open(gif_path, 'rb') as f:
            gif_data = f.read()
        st.image(gif_data, key="gif_test", width=0, height=0)  # Hidden test load
    except Exception as e:
        st.warning(f"GIF test load failed: {e}")
else:
    st.warning("GIF file 'IMG_8286.gif' not found in the repository root. Using fallback transition.")
...
# Function to trigger GIF transition
def trigger_transition():
    if not st.session_state.transition_triggered:
        st.session_state.transition_triggered = True
        st.markdown("""
        <script>
            // Trigger transition with slight delay to ensure state update
            setTimeout(() => {
                showTransition(() => {
                    // This will trigger Streamlit rerun
                    window.location.reload(); // Force full reload for clean transition
                });
            }, 100);
        </script>
        """, unsafe_allow_html=True)
...
# Enhanced JavaScript for button click detection
st.markdown("""
<script>
    // Enhanced transition trigger for all navigation buttons
    document.addEventListener('click', function(e) {
        const button = e.target.closest('button');
        if (button) {
            const buttonText = button.textContent.trim();
            const shouldTransition = 
                buttonText.includes('User Portal') || 
                ...
                buttonText.includes('Submit Callback');
            ...
            if (shouldTransition) {
                ...
                setTimeout(() => {
                    showTransition(() => {
                        // Force Streamlit to rerun after transition
